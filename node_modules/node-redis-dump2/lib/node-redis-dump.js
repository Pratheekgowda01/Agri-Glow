...
RedisDump.prototype.import = function(params) {
  ...
  this._importDumpBase64 = async function(callback) {
    const items = params.data.split(/(RESTORE|RESTOREB64)\\s/g);
    const regExpArgs = /"?(.+?)"?\s+"?(\d+)"?\s+["']?([a-z\d\/+=]+?)["']?\\s*$/i;
    let data;
    const promises = [];
      ...
      try {
        switch (command) {
          ...
          case 'RPUSHX':
            ...
            callArgs = args.match(/"?(.+?)"?\s+(["'](.+?)["']|(.+?))(?:\s|$)/i).slice(1, 3);
          ...
          case 'LSET':
            ...
            callArgs = args.match(/"?(.+?)"?\s+"?(\d+?)"?\s+(["'](.+?)["']|(.+?))(\s|$)/i).slice(1, 4);
            callArgs[2] = callArgs[2].replace(/(^["']|["']$)/g, '');   // remove optional quotes around value string
            break;

          case 'HSET':
            callArgs = args.match(/"?(.+?)"?\s+"?(.+?)"?\s+(["'](.+?)["']|(.+?))\s*$/i).slice(1, 4);
            callArgs[2] = callArgs[2].replace(/(^["']|["']$)/g, '');   // remove optional quotes around value string
            break;

          case 'LTRIM':
            // command "LTRIM key 1 1"
            callArgs = args.match(/"?(.+?)"?\s+(\d+?)\s+(\d+?)\s*$/i).slice(1, 4);
            break;
